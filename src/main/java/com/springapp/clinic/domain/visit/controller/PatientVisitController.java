...
public class PatientVisitController {
    ...
    public VisitDto createVisit(@Valid @RequestBody VisitDto visitDto, @PathVariable Long patientId, @RequestParam(required = false) Long doctorId) {
        log.info("Add visit {}", visitDto);
        return visitService.createVisit(patientId, visitDto, doctorId);
    }

    @PutMapping("/{patientId}/visits/{visitId}")
    public VisitDto updateVisit(@RequestBody VisitDto newVisitDto, @PathVariable Long patientId, @PathVariable Long visitId, @RequestParam(required = false) Long doctorId) {
        ...
        return visitService.getSingleVisitById(patientId, visitId)
                .map(visitDto -> {
                    ...
                    return visitService.updateVisit(patientId, visitDto, doctorId);
                }).orElseThrow(() -> new NotFoundException("Not found entity by id: " + visitId));
    }
